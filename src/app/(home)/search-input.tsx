...
import { SearchIcon, X as XIcon } from "lucide-react"; // Import XIcon from lucide-react
...
export const SearchInput = () => {
  const [search, setSearch] = useSearchParam(""); // Manages the search query in URL or state
  const [value, setValue] = useState(search); // Tracks the current input value
  ...
  const handleClear = () => {
    setValue("");
    setSearch("");
    inputRef.current?.blur(); // Removes focus from the input field
  };

  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setSearch(value); // Updates the search query in state
